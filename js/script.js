$(document).ready(function(){
    ...
    function afficheVehicule() {
        $.post ('models/ajax.php',  // URL du dossier où s'effectue le traitement
                ...
                function (donnees) { //traitement de la réponse donnee=toto=ajax.php->echo json_encode($tab);
    ...
    $('#vehicule').on('submit', function(e){
        ...
        ($('#marque').val().length==0) ? erreurForm += '<div>la marque ne peut pas être vide</div>' : params += '&marque='+$('#marque').val();
        ($('#modele').val().length==0) ? erreurForm += '<div>le modèle ne peut pas être vide</div>' : params += '&modele='+$('#modele').val();
        ($('#annee').val().length==0) ?  erreurForm += '<div>l\'année ne peut pas être vide</div>' : params += '&annee='+$('#annee').val();
        ($('#couleur').val().length==0) ? erreurForm += '<div>la couleur ne peut pas être vide</div>' : params += '&couleur='+$('#couleur').val();
